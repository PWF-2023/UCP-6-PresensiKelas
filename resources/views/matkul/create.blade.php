        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="post" action="{{ route('matkul.store') }}" class="">
                        @csrf
                        @method('post')
                        <div class="mb-6">
                            <x-input-label for="matkul" :value="__('matkul')"/>
                            <x-text-input id="matkul" name="matkul" type="text" class="block w-full mt-1" required autofocus aoutocomplete="matkul"/>
                            <x-input-error class="mt-2" :messages="$errors->get('matkul')"/>
                        </div>
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Save') }}</x-primary-button>
                            <a href="{{ route('matkul.index') }}" class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest
